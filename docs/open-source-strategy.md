...
A compact React UI kit for developer tools, built from Dao's HeroUI-backed component layer and Tailwind CSS.
...
It should not be extracted while Dao is still deciding its core UI patterns. The app should first validate HeroUI-backed Dao components, Dao design tokens, density, forms, command palette, sidebar, panel, and dialog patterns in real product surfaces.
...
```txt
Extracted and maintained reusable infrastructure packages from Dao, including an Electron-Go service bridge and a HeroUI-backed React component system, both validated through continuous use in the main product.
```
