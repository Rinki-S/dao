    ...
    <section id="workspaces" className="max-w-3xl rounded-lg border border-[#E5E7EB] bg-white p-5">
      <div className="mb-5 flex items-center justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-[#6B7280]">Create the first local container for Dao.</p>
        </div>
      </div>
      ...
      {status === 'loading' && <p className="text-sm text-[#6B7280]">Loading workspaces...</p>}
      ...
      {status === 'ready' && workspaces.length > 0 && (
        <ul className="grid gap-2">
          {workspaces.map((workspace) => (
            <li key={workspace.id} className="rounded-md border border-[#E5E7EB] px-3 py-3">
              <strong className="block text-sm font-medium text-[#111827]">{workspace.name}</strong>
